...
implicit none
    ...
    t=0.0
        ...
        DO j=1,rep
            ...
            DO k=1,(n-1)
                ...
                rab(2)=rab(2)-ANINT(rab(2)) ! condiciones periodicas
                ...
                bij    = DOT_PRODUCT ( rab, vab )   ! obtenemos el producto escalar (ri-rj)*(vi-vj)
                    ...
                    IF (v(k,2)>0 ) THEN
                        tcol=(H-sigma*0.5-r(k,2)*H)/v(k,2)
                        IF (tcol<0) THEN 
                    ...
                    IF (k==1) THEN
                        ...
                        IF( discr>0.0) THEN ! si colisiona con la sucesiva particula
                            tcol = ( -bij - SQRT ( discr ) ) / ( SUM ( vab**2 ) )
                             ...
                             !!PRINT*, 'colisión:',k,n,'tiempo',tcol
            ...
            t=t+colt
            r(:,1)     = r(:,1) + colt * v(:,1)/longy 
            r(:,2)     = r(:,2) + colt* v(:,2)/H   ! Advance all positions by t (box=1 units)
            r(:,1)     = r(:,1) - ANINT ( r(:,1) ) ! Apply periodic boundaries
            ...
            END IF
               ...
               END DO
                    tmp(j,1)=sumv(i,j,1)/(temp+tempz)
                    tmp(j,2)=sumv(i,j,2)/(temp+tempz)
    ...
    CONTAINS
      ...
      SUBROUTINE collide ( a, b)
        ...
        rij(:) = r(a,:) - r(b,:)
        rij(2) = rij(2) - ANINT ( rij(2) ) ! Separation vector
        rab(1) = rab(1) *longy ! unidades correctas
        rab(2)=rab(2)*(H)
     ...
     SUBROUTINE superpuesto() 
        ...
        iloop:DO q=1,n
            IF(ABS(r(q+1,1)-r(q,1))<sigma) THEN
